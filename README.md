...
```bash
aws-pdf-3$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-pdf-3$ sam local start-api
aws-pdf-3$ curl http://localhost:3000/
```
...
```bash
aws-pdf-3$ sam logs -n HelloWorldFunction --stack-name aws-pdf-3 --tail
```
...
```bash
sam delete --stack-name aws-pdf-3
```
